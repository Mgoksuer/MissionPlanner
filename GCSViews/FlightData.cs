...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        {
            // VLCControl nesnesini oluştur
            vlcPlayer = new Vlc.DotNet.Forms.VlcControl();
            vlcPlayer.BeginInit(); // Başlatma işlemlerine başla

            // VLCControl'ü pnlCameraDisplay (kamera görüntüsü paneliniz) içine ekle
            this.pnlCameraDisplay.Controls.Add(vlcPlayer);
            // VLCControl'ün pnlCameraDisplay'ı tamamen doldurmasını sağla
            vlcPlayer.Dock = DockStyle.Fill;

            // VLC libvlc.dll dosyalarının konumunu belirtme
            var vlcInstallPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "VideoLAN", "VLC");
            if (!Directory.Exists(vlcInstallPath))
            {
                vlcInstallPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "VideoLAN", "VLC");
            }

            if (Directory.Exists(vlcInstallPath))
            {
                vlcPlayer.VlcLibDirectory = new DirectoryInfo(vlcInstallPath);
            }
            else
            {
                MessageBox.Show("VLC Media Player kurulum dizini bulunamadı. Lütfen VLC'nin bilgisayarınızda kurulu olduğundan veya libvlc dosyalarını Mission Planner'ın çalıştığı dizine ('MissionPlanner.exe'nin yanına bir 'libvlc' klasörü açıp içine) kopyaladığınızdan emin olun.", "VLC Hatası", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // BU KISIMDAKİ HATAYI DÜZELTİYORUZ:
            // vlcPlayer.VlcMediaplayerOptions.Add(...) yerine, seçenekleri doğrudan SetMedia metoduna
            // veya SetMedia çağrılmadan önce bir listeye ekleyeceğiz.

            // Seçenekleri bir liste olarak tanımlayın.
            ...
            // InitializeVlcPlayer metodunda vlcPlayer.SetMedia() çağrısı yapılmadığı için,
            // bu seçenekler doğrudan player'a atanmaz. Sadece SetMedia çağrısında kullanılırlar.
            // Eğer VLC'ye global seçenekler atamak isterseniz, bu genellikle constructor veya bir ayar metoduyla yapılır.
            // Ancak en yaygın kullanım SetMedia ile beraberdir.

            vlcPlayer.EndInit(); // Oynatıcıyı başlat
        }
        ...
        {
            // txtRTSPAddress isimli metin kutusundan RTSP adresini al, başındaki ve sonundaki boşlukları temizle
            string rtspAddress = txtRTSPAddress.Text.Trim();

            if (!string.IsNullOrEmpty(rtspAddress)) // Eğer RTSP adresi boş değilse
            {
                try
                {
                    if (vlcPlayer.IsPlaying) // Eğer VLC oynatıcı zaten bir yayın oynatıyorsa
                    {
                        vlcPlayer.Stop(); // Önceki yayını durdur
                    }

                    // VLC oynatıcısına medya kaynağını (RTSP adresi) ayarla
                    vlcPlayer.SetMedia(new Uri(rtspAddress));
                    // Yayını oynatmaya başla
                    vlcPlayer.Play();
                }
                catch (Exception ex) // Bağlantı sırasında bir hata oluşursa
                {
        ...
        {
            if (vlcPlayer != null) // Eğer VLC oynatıcısı nesnesi oluşturulmuşsa
            {
                if (vlcPlayer.IsPlaying) // Eğer şu anda bir yayın oynatılıyorsa
                {
                    vlcPlayer.Stop(); // Yayını durdur
                }
                vlcPlayer.Dispose(); // VLC kaynaklarını serbest bırak
                vlcPlayer = null; // Nesneyi null yaparak bellekte yer kaplamamasını sağla
            }
        }
    }
}
